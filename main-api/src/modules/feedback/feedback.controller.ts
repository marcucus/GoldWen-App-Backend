...
export class FeedbackController {
  ...
  async create(@Req() req: Request, @Body() dto: CreateFeedbackDto) {
    const feedback = await this.repository.save(this.repository.create({ ...dto, userId: (req.user as User).id }));
    return { success: true, data: { id: feedback.id, createdAt: feedback.createdAt } };
  }
  @Get('admin')
  @UseGuards(AdminGuard)
  @ApiOperation({ summary: 'Read the latest 100 feedback submissions (admin)' })
  async list() {
    return { success: true, data: await this.repository.find({ order: { createdAt: 'DESC' }, take: 100 }) };
  }
}
