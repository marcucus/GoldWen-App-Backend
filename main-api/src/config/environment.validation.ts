...
export function validateEnvironment(
  ...
  for (const name of [
    ...
    if (value(name)) {
      try {
        ...
        errors.push(`${name} must be an HTTP URL`);
      }
    }
  }
  ...
  if (value('NODE_ENV') === 'production') {
    ...
    if (value('STORAGE_PROVIDER') !== 's3')
      errors.push('Production requires STORAGE_PROVIDER=s3');
    requireValue('S3_BUCKET');
    for (const name of ['APP_URL', 'FRONTEND_URL', 'WEB_URL']) {
      if (value(name) && !value(name).startsWith('https://'))
        errors.push(`${name} requires HTTPS in production`);
    }
    ...
    if (value('EMAIL_PROVIDER') === 'sendgrid')
      requireValue('SENDGRID_API_KEY');
    else
      for (const name of ['EMAIL_HOST', 'EMAIL_USER', 'EMAIL_PASSWORD'])
        requireValue(name);
  }
  if (errors.length)
